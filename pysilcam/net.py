...
class Net:
    ...
    def __init__(self, name='LeNet', input_width=32, input_height=32, input_channels=3, num_classes=7,
                 learning_rate=0.01,
...
class CoapNet(Net):
    ...
    def build_model(self, model_file):
        ...
        # Layer 2:
        print('Layer 2:')
        # 2: Convolution layer with 128 filters size 3 and stride 1
        print('2: Convolution again')
        ...
        print('Layer 5: Fully-connected 512 node neural network')
        net = fully_connected(net, 512, activation='relu')
        # Step 6: Dropout - throw away some data randomly during training to prevent over-fitting
        # print('Step 6: Dropout - throw away some data randomly during training to prevent over-fitting')
        # net = dropout(net, self.keep_prob)
        print('Layer 6: Fully-connected 256 node neural network')
        net = fully_connected(net, 256, activation='relu')
        # Step 7: Dropout - throw away some data randomly during training to prevent over-fitting
        # print('Step 7: Dropout - throw away some data randomly during training to prevent over-fitting')
        # net = dropout(net, self.keep_prob)
        print('Layer 7: Fully-connected 256 node neural network')
        net = fully_connected(net, 256, activation='relu')
        ...
        # Layer 8: Fully-connected with outputs to make the final prediction
        print('Layer 8: Fully-connected softmax with outputs to make the final prediction')
        net = fully_connected(net, self.num_classes, activation='softmax')

        # Layer 8: Fully-connected with outputs to make the final prediction
        print('optimizer = adam learning_rate %f, loss=categorical_crossentropy'%self.learning_rate)

        net = regression(net, optimizer='adam', learning_rate=self.learning_rate,
                         loss='categorical_crossentropy', name='target')

        # Wrap the network in a model object
        print('Wrap the network in a model object tensorboard_verbose=3')
        model = tflearn.DNN(net, tensorboard_verbose=3, checkpoint_path=self.check_point_file)
...
class GoogleNet(Net):
    ...
    def build_model(self, model_file):
        ...
        # fc
        print(" fc ")
        loss = fully_connected(pool5_7_7, self.num_classes, activation='softmax')
...
class ResNet(Net):
    ...
    def build_model(self, model_file):
        ...
        # Regression
        net = tflearn.fully_connected(net, self.num_classes, activation='softmax')
