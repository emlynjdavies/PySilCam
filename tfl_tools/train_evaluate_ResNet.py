...
LOG_FILE = os.path.join(MODEL_PATH, 'cvResNet.out')
...
ResNet = Net(name, input_width, input_height, input_channels, num_classes, learning_rate,
                momentum, keep_prob)
fh = open(LOG_FILE, 'w')
fh.write(name)
print(name)
for trainX, trainY, testX, testY in data_set.gen():
    ...
    model, conv_arr = ResNet.build_model(model_file)

    # Training
    print("start training round %f ...", i)
    ResNet.train(model, trainX, trainY, testX, testY, round_num, n_epoch, batch_size)
    ...
    y_pred, y_true, acc, pre, rec, f1sc, conf_matrix, norm_conf_matrix = \
        ResNet.evaluate(model, testX, testY)
